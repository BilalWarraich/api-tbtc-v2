...
router.get('/trackget/:hash', async function (request, response) {
	var ResponseCode = 200;
	var ResponseMessage = ``;
	var ResponseData = null;
		...
		if(request.params) {
			if (!request.params.hash) {
				...
				if (hash.length == 64) {
					...
					xmlHttp.open( "GET", 'https://api.blockcypher.com/v1/btc/test3/txs/' + hash, false ); // false for synchronous request
					xmlHttp.send();
...
router.get('/getBalance/:walletAddress', async function (request, response) {
	...
	var finalResponse = null;
	let errors = null;
	console.log(request.params,"AAAAAAAAAAAA");
		...
		if(request.params) {
			if (!request.params.walletAddress) {
	           ...
	           // }
				...
				await axios.get(
					`https://api.blockcypher.com/v1/btc/test3/addrs/${request.params.walletAddress}/balance`
				    ).then(res => {
